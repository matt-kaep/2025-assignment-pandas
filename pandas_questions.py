...
def compute_referendum_result_by_regions(referendum_and_areas):
    ...
    grouped = referendum_and_areas.groupby(
        by=['code_reg', 'name_reg']).sum(numeric_only=True).reset_index()
    result = grouped[['code_reg', 'name_reg',
                      'Registered', 'Abstentions', 'Null', 'Choice A',
                      'Choice B']]
...
def plot_referendum_map(referendum_result_by_regions):
    ...
    * Return a gpd.GeoDataFrame with a column 'ratio' containing the results.
    ...
    ax = gdf.plot(column='ratio', legend=True, cmap='OrRd', edgecolor='black')
    ax.set_title("Referendum results by region: rate of 'Choice A'")
    plt.axis('off')
    return gdf
